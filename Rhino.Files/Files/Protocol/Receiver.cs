...
namespace Rhino.Files.Protocol
{
    public class Receiver : IDisposable
    {
        ...
        private IEnumerator<int> ProcessRequest(FileWatcherFile client, AsyncEnumerator ae)
        {
            try
            {
                using (var stream = client.GetStream())
                {
                    ...
                    catch (Exception exception) { _logger.Warn("Unable to read length data from " + sender, exception); yield break; }
                    yield return 1;
                    ...
                    if (lengthOfDataToRead < 0) { _logger.WarnFormat("Got invalid length {0} from sender {1}", lengthOfDataToRead, sender); yield break; }
                    ...
                    try { messages = SerializationExtensions.ToMessages(buffer); _logger.DebugFormat("Deserialized {0} messages from {1}", messages.Length, sender); }
