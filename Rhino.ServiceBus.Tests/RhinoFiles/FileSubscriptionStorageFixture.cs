...
{
    public class FileSubscriptionStorageFixture : IDisposable, OccasionalConsumerOf<string>
    {
        private readonly FileSubscriptionStorage storage;

        public FileSubscriptionStorageFixture()
        {
            if (Directory.Exists("test.esent"))
                Directory.Delete("test.esent", true);

            storage = new FileSubscriptionStorage("test.esent",
        ...
        public void CanAddRemoteInstanceSubscription()
        {
            storage.ConsumeAddInstanceSubscription(
                ...
                {
                    Endpoint = new Uri("file://localhost/foobar").ToString(),
                    InstanceSubscriptionKey = Guid.NewGuid(),
                    Type = typeof(string).ToString()
                });

            Assert.Equal(
                new Uri("file://localhost/foobar"),
